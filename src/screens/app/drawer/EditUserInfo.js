...
import {Avatar, Accessory, Icon, Button, Header, Input} from 'react-native-elements';
import Toast from 'react-native-simple-toast';
import Modal from 'react-native-modal';

import scale from '../../../common/Scale';
import {isEmpty, replaceEmail, _chkPwd, isSNS} from '../../../common/Utils';
...
const EditUserInfo = props => {
  ...
  const {userEmail, userName, userPhone, userImage, userImageNo, setUserInfo, resetUserInfo} = useContext(
    UserTokenContext
  );
    ...
    try {
      if (userImage !== userInfo.image) {
        if (userInfo.image !== '') {
          ...
          console.log('_onEditUserInfo -> imageUploadResult: ', imageUploadResult.image_no);
      ...
      Toast.showWithGravity('회원정보가 수정되었습니다.', Toast.LONG, Toast.CENTER);
    } catch (error) {
      console.log('_onEditUserInfo -> error', error);
      Alert.alert(
        '[안내]',
        '사진을 처리하는 도중 에러가 발생했습니다.\n다시 시도해 주세요.',
        [{text: '확인', onPress: () => console.log('OK Pressed'), style: 'cancel'}],
