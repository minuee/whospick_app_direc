...
const ProfileReviewDetail = props => {
  ...
  const _getRequestProfileDetail = async () => {
    try {
      const apiResult = await apiObject.getReviewActorProfile({eval_apply_no: props.route.params.eval_apply_no});

      setDataList(apiResult);
    } catch (error) {
      console.log('_getRequestProfileDetail -> error', error);
      ...
      <SafeAreaView style={{...styles.contents}}>
        <ScrollView>
            ...
            <View style={{...styles.viewUpActorInfo}}>
              ...
              <Text
                style={{
                  ...
                  fontSize: scale(12),
                  flex: 1,
                }}>{`${dataList.age}세     ${dataList.height}cm/${dataList.weight}kg`}</Text>
          ...
          <View style={{...styles.viewActorStarArea}}>
            ...
            <FastImage
              source={
                isEmpty(dataList.director_image)
                  ? require('../../../../assets/images/drawable-xxxhdpi/profile.png')
                  : {uri: dataList.director_image}
              }
