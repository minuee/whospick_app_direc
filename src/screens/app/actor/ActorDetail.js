...
import {AddComma, changeTime, getAge, isEmpty, isEmptyArr, screenWidth} from '../../../common/Utils';
...
const ActorDetail = props => {
  ...
  const _getActorInfo = async () => {
    try {
      ...
      console.log('_getActorInfo -> error', error);
          ...
          <View style={{...styles.viewScrollInner}}>
            <View style={{...styles.viewInfoArea}}>
              ...
              <View style={{...styles.viewInfoRow}}>
                ...
                <Text style={{...styles.txtRowBody}}>{`${getAge(dataList.birth_dt)}세/${changeTime(
                  dataList.birth_dt
                )}`}</Text>
              </View>
              <View style={{...styles.viewInfoRow}}>
              ...
              <View style={{...styles.viewInfoRow}}>
                ...
                <Text
                  style={{
                    ...styles.txtRowBody,
                  }}>{`상의${dataList.top}/하의${dataList.bottom}/신발${dataList.shoes}`}</Text>
              </View>
              <View style={{...styles.viewInfoRow}}>
              ...
              {/* {!isEmpty(dataList.actorSongType.id) ? (
                <View style={{...styles.viewInfoRow}}>
                  <Text style={{...styles.txtRowLabel}}>음역대</Text>
                  <Text style={{...styles.txtRowBody}}>{dataList.actorSongType.name}</Text>
                </View>
              ) : null} */}
            </View>
            ...
            <View style={{...styles.viewInfoArea}}>
              <View style={{...styles.viewInfoRow}}>
                ...
                <TouchableOpacity
                  style={{flex: 1}}
                  onPress={async () => {
                    !isEmpty(dataList.video_url) && (await Linking.openURL(dataList.video_url));
                  }}>
                  <Text
                    style={{...styles.txtRowBody, color: '#e5293e', textDecorationLine: 'underline'}}
                    numberOfLines={1}>{`${isEmpty(dataList.video_url) ? '없음' : dataList.video_url}`}</Text>
                </TouchableOpacity>
              </View>
              {dataList.your_applicant && (
                <View style={{...styles.viewInfoRow}}>
                  <Text style={{...styles.txtRowLabel}}>SNS주소</Text>
                  {isEmpty(dataList.facebook) && isEmpty(dataList.instagram) && isEmpty(dataList.twitter) && (
                    <Text style={{...styles.txtRowBody, color: '#e5293e', textDecorationLine: 'underline'}}>없음</Text>
                  )}
            ...
            {dataList.your_applicant && (
              <View style={{...styles.viewInfoArea}}>
                <Text style={{...styles.txtInfoLabel}}>연락처정보</Text>
                <View style={{...styles.viewInfoRow}}>
                  <Text style={{...styles.txtRowLabel}}>이메일주소</Text>
                  <TouchableOpacity
                    onPress={async () => {
                      await Linking.openURL(`mailto:${dataList.email}`);
                    }}>
                    <Text
                      style={{...styles.txtRowBody, color: '#e5293e', textDecorationLine: 'underline'}}
                      numberOfLines={1}>{`${dataList.email}`}</Text>
                  </TouchableOpacity>
                </View>
                <View style={{...styles.viewInfoRow}}>
                  <Text style={{...styles.txtRowLabel}}>전화번호</Text>
                  <TouchableOpacity
                    onPress={async () => {
                      await Linking.openURL(`tel:${dataList.mobile_no}`);
                    }}>
                    <Text
                      style={{...styles.txtRowBody, color: '#e5293e', textDecorationLine: 'underline'}}
                      numberOfLines={1}>{`${dataList.mobile_no}`}</Text>
                  </TouchableOpacity>
                </View>
              </View>
            )}
