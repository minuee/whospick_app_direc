...
import {changeTime, isEmpty, timeForToday} from '../../../common/Utils';
...
const PartnerDetail = props => {
  ...
  const _getPartnerDetail = async () => {
    try {
      ...
      console.log('_getPartnerDetail -> error', error);
  ...
  const _onPostComment = () => {
    ...
    if (!isEmpty(inputSubIndex)) {
      try {
        ...
        // tmpArr[inputSubIndex].reply.push({
        //   profile_image: userImage,
        //   user_name: userName,
        //   content: inputText,
        //   reg_dt: +new Date() / 1000,
        // });
        // setDataList({...dataList, comment: tmpArr});
      } catch (error) {
        console.log('_onPostComment -> inputSubIndex -> error', error);
        Toast.show('네트워크 통신 중 오류가 발생했습니다.\n잠시 후 다시 시도해주세요.', Toast.SHORT);
      }
    } else {
      try {
        ...
        console.log('_onPostComment -> error', error);
        Toast.show('네트워크 통신 중 오류가 발생했습니다.\n잠시 후 다시 시도해주세요.', Toast.SHORT);
      }
    }
    setInputText('');
            ...
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              <Text
                style={{
                  fontSize: scale(10),
                  color: '#bababa',
                }}>{`${changeTime(postInfo.uploadReg)}`}</Text>
            </View>
              ...
              <View key={`comment_${index}`}>
                <View style={{...styles.viewCommentArea}}>
                  <FastImage
                    source={
                      isEmpty(item.profile_image)
                        ? require('../../../../assets/images/drawable-xxxhdpi/profile.png')
                  ...
                  <View style={{flex: 1}}>
                    ...
                    <View style={{flexDirection: 'row', alignItems: 'center'}}>
                      <Text style={{...styles.txtCommentUploadReg, marginRight: scale(10)}}>{`${timeForToday(
                        item.reg_dt
                      )}`}</Text>
        ...
        <View style={{...styles.viewInputArea}}>
          {!isEmpty(inputSubIndex) ? (
            <View style={{...styles.viewInputSubArea}}>
              ...
              <TouchableOpacity
                onPress={() => {
                  setInputSubIndex(null);
                  setInputSubActorName('');
                }}>
                <Text>취소</Text>
              </TouchableOpacity>
            </View>
